...
*/
void WAV_write_file(char *file_name, int16_t *buffer, uint32_t num_samples)
{
	FILE	*wav_fp;
	...
	wav_init_header(&header, 1, 32000, num_samples * 2);
	fwrite(&header, sizeof(header), 1, wav_fp);

	// audio data
	fwrite(buffer, num_samples * 2, 1, wav_fp);
	fclose(wav_fp);
}
